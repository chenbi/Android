...
public class RobotListener extends KeyAdapter implements GLEventListener {
    ...
    /** Rotation angle of entire arm based on ccordinate system of base */
    public double rotBottom = 0;
    /** Rotation angle of middle and upper arm based on ccordinate system of lower arm  */
    public double rotMid = 0;
    /** Rotation angle of upper arm based on ccordinate system of middle arm  */
    public double rotTop = 0;
    /** Amount angle is changed each time a putton is pressed. */
    public double dAngle = 5;
    
    // These are only used when we want to save the image to a jpg file
    int frameWidth = 640, frameHeight = 480;
    boolean saveImage = false; // change this to true if you want each frame saved.
    int imageIndex = 0;  
    ...
    public void display(GLAutoDrawable d) {
        ...
        // This is only used if you want to dump the frame buffer out to jpg files. 
        // Be careful because this will save *every* time display() is called. You may
        // end up with a lot of files.  The jpg files can be used to create an 
        // animation but you will need to use video editing software. 
        if (saveImage) {
            SaveJPGImage.saveFrameBuffer("images\\robot", frameWidth, frameHeight, gl, imageIndex);
            imageIndex++;
            System.out.println("rotBase = " + rotBase + "  rotBottom = "+ 
                    rotBottom + "  rotMid = " + rotMid + "  rotTop = " + rotTop);
        }
    }
